...
Le regole sono utilizzate nei controllori fuzzy per simulare la conoscenza che aiuta l'agente nell'operare le proprie scelte. Sono date come una sequenza di \emph{if-then} del tipo "if $X$ is $M_n$, then $Y$ is $N_m$" dove $M_n$ e $N_m$ sono intervalli. Collettivamente le regole possono essere rappresentate come l'unione $S$ dei vari intervalli:
$$
S = \cup M_i \times N_i
$$
 In congiunzione con un fatto della forma "$X$ is $M_n$" ricevuto come input, la logica interna al fuzzy control dedurrà che "$Y$ is $N_m$". Le regole possono assumere come valori intervalli \emph{crisp} come in Figura \ref{fig:31} (a), oppure valori fuzzy come in Figura \ref{fig:31} (b). In ogni caso, dato un input $x_0$ l'ouput verrà calcolato come la composta del singoletto di $x_0$ e l'unione degli intervalli $S$ (in simboli, $\{x_0\} \circ S$) come in Figura \ref{fig:32}. In alcune applicazioni è utile considerare non solo l'unione di regole, ma anche la loro congiunzione. 
